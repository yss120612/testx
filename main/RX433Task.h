...
class RX433Task : public Task
{
public:
RX433Task(const char *name, uint32_t stack, QueueHandle_t q, gpio_num_t data):
...
void setup() override;
void loop() override;
static void timer_callback_static(TimerHandle_t tm);
void timer_callback();
QueueHandle_t que;
gpio_num_t data_pin;
TimerHandle_t _timer;

static inline volatile uint32_t _receivedValue {};
static inline volatile uint16_t _receivedBitlength {};
//static volatile uint16_t _receivedDelay;
//static volatile uint16_t _receivedProtocol;
static inline uint16_t _timings[RX433_SWITCH_MAX_CHANGES]{};
};
